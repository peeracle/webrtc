...
{
  'variables': {
    'keystore_path%': '<(webrtc_depth)/build/android/ant/chromium-debug.keystore',
  ...
  'inputs': [
    '<(webrtc_depth)/build/android/gyp/finalize_splits.py',
    '<(webrtc_depth)/build/android/gyp/finalize_apk.py',
    '<(webrtc_depth)/build/android/gyp/util/build_utils.py',
    '<(keystore_path)',
  ],
  'action': [
    'python', '<(webrtc_depth)/build/android/gyp/finalize_splits.py',
  ...
  'conditions': [
    ...
    ['language_splits != []', {
      ...
      'inputs': [
        "<!@(python <(webrtc_depth)/build/apply_locales.py '<(resource_packaged_apk_path)_ZZLOCALE' <(language_splits))",
      ],
      'outputs': [
        "<!@(python <(webrtc_depth)/build/apply_locales.py '<(base_output_path)-lang-ZZLOCALE.apk' <(language_splits))",
      ],
      'action': [
