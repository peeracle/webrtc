...

{
  'conditions': [
    ['OS=="win" and chromium_win_pch==1', {
        'target_defaults': {
          'msvs_precompiled_header': '<(webrtc_depth)/build/precompile.h',
          'msvs_precompiled_source': '<(webrtc_depth)/build/precompile.cc',
          'sources': ['<(webrtc_depth)/build/precompile.cc'],
        }
      }],
  ],
}
