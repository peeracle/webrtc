...
#  app_manifest_version_code - set the apps version number.
{
  'variables': {
    ...
    'resource_dir%':'<(webrtc_depth)/build/android/ant/empty/res',
    'R_package%':'',
    ...
    'native_libraries_template': '<(webrtc_depth)/base/android/java/templates/NativeLibraries.template',
  ...
  'conditions': [
    ...
    ['native_lib_target != "" and android_must_copy_system_libraries == 1', {
      'dependencies': [
        '<(webrtc_depth)/build/android/setup.gyp:copy_system_libraries',
      ],
    }],
    ['use_chromium_linker == 1', {
      'dependencies': [
        '<(webrtc_depth)/base/base.gyp:chromium_android_linker',
      ],
    }],
    ['enable_errorprone == 1', {
      'dependencies': [
        '<(webrtc_depth)/third_party/errorprone/errorprone.gyp:require_errorprone',
      ],
    }],
    ['native_lib_target != ""', {
      'variables': {
      ...
      'actions': [
        ...
        },
        {
          ...
          'inputs': [
            '<(webrtc_depth)/build/android/gyp/util/build_utils.py',
            '<(webrtc_depth)/build/android/gyp/gcc_preprocess.py',
          ...
          'action': [
            'python', '<(webrtc_depth)/build/android/gyp/gcc_preprocess.py',
            '--include-path=',
        ...
        },
        {
          ...
          'inputs': [
            '<(webrtc_depth)/build/android/gyp/create_placeholder_files.py',
          ],
          'outputs': [
            '<(native_lib_placeholder_stamp)',
          ],
          'action': [
            'python', '<(webrtc_depth)/build/android/gyp/create_placeholder_files.py',
      ...
      'conditions': [
        ['gyp_managed_install == 1', {
          ...
          'dependencies': [
            '<(webrtc_depth)/build/android/setup.gyp:get_build_device_configurations',
            '<(webrtc_depth)/build/android/pylib/device/commands/commands.gyp:chromium_commands',
          ],
          'actions': [
            {
            ...
            },
            {
              ...
              'inputs': [
                '<(webrtc_depth)/build/android/gyp/util/build_utils.py',
                '<(webrtc_depth)/build/android/gyp/create_device_library_links.py',
                '<(apk_install_record)',
              ...
              'action': [
                'python', '<(webrtc_depth)/build/android/gyp/create_device_library_links.py',
    ...
    ['gyp_managed_install == 0 or create_standalone_apk == 1 or create_abi_split == 1', {
      'dependencies': [
        '<(webrtc_depth)/build/android/rezip.gyp:rezip_apk_jar',
      ],
    }],
      ...
      'actions': [
        {
          ...
          'inputs': [
            '<(webrtc_depth)/build/android/gyp/util/build_utils.py',
            '<(webrtc_depth)/build/android/gyp/generate_split_manifest.py',
          ...
          'action': [
            'python', '<(webrtc_depth)/build/android/gyp/generate_split_manifest.py',
    ...
    ['gyp_managed_install == 1', {
      'actions': [
        ...
        },
        {
          ...
          'inputs': [
            '<(webrtc_depth)/build/android/gyp/util/build_utils.py',
            '<(webrtc_depth)/build/android/gyp/apk_install.py',
            '<(build_device_config_path)',
            '<(incomplete_apk_path)',
          ],
          'outputs': [
          ...
          'action': [
            'python', '<(webrtc_depth)/build/android/gyp/apk_install.py',
            '--build-device-configuration=<(build_device_config_path)',
          ...
          'conditions': [
            ...
            ['language_splits != []', {
              'inputs': [
                "<!@(python <(webrtc_depth)/build/apply_locales.py '<(final_apk_path_no_extension)-lang-ZZLOCALE.apk' <(language_splits))",
              ],
              'action': [
                "<!@(python <(webrtc_depth)/build/apply_locales.py -- '--split-apk-path=<(final_apk_path_no_extension)-lang-ZZLOCALE.apk' <(language_splits))",
              ],
            }],
          ],
        },
    ...
    ['is_test_apk == 1', {
      'dependencies': [
        '<(webrtc_depth)/build/android/pylib/device/commands/commands.gyp:chromium_commands',
        '<(webrtc_depth)/tools/android/android_tools.gyp:android_tools',
      ]
    }],
    ['run_findbugs == 1', {
      'actions': [
        {
          ...
          'inputs': [
            '<(webrtc_depth)/build/android/findbugs_diff.py',
            '<(webrtc_depth)/build/android/findbugs_filter/findbugs_exclude.xml',
            '<(webrtc_depth)/build/android/pylib/utils/findbugs.py',
            '>@(input_jars_paths)',
            '<(jar_path)',
            '<(compile_stamp)',
          ],
          ...
          'action': [
            'python', '<(webrtc_depth)/build/android/findbugs_diff.py',
  ...
  'dependencies': [
    '<(webrtc_depth)/tools/android/md5sum/md5sum.gyp:md5sum',
  ],
  'actions': [
    {
      ...
      'inputs': [
        '<(webrtc_depth)/build/android/gyp/util/build_utils.py',
        '<(webrtc_depth)/build/android/gyp/process_resources.py',
        '<(android_manifest_path)',
        '>@(additional_input_paths)',
        '>@(resource_input_paths)',
        '>@(dependencies_res_zip_paths)',
      ...
      'action': [
        'python', '<(webrtc_depth)/build/android/gyp/process_resources.py',
    ...
    },
    {
      ...
      'inputs': [
        '<(webrtc_depth)/build/android/gyp/util/build_utils.py',
        '<(webrtc_depth)/build/android/gyp/javac.py',
        '>@(java_sources)',
        '>@(input_jars_paths)',
        '<(codegen_stamp)',
        '<@(extra_inputs)',
      ...
      'action': [
        'python', '<(webrtc_depth)/build/android/gyp/javac.py',
    ...
    },
    {
      ...
      'inputs': [
        '<(webrtc_depth)/build/android/gyp/apk_obfuscate.py',
        '<(webrtc_depth)/build/android/gyp/util/build_utils.py',
      ...
      'action': [
        'python', '<(webrtc_depth)/build/android/gyp/apk_obfuscate.py',
        ...
        '--proguard-jar-path', '<(webrtc_depth)/third_party/proguard/lib/proguard.jar',
      ...
      'variables': {
        ...
        'conditions': [
          ...
          ['create_abi_split == 0', {
            ...
            'native_libs_dir': '<(webrtc_depth)/build/android/ant/empty/res',
          }],
        ],
      },
      'includes': ['android/apkbuilder_action.gypi'],
