...
# since gyp_chromium is automatically forcing its inclusion.
{
  ...
  'variables': {
    ...
    'variables': {
      'variables': {
        ...
        'conditions': [
          ...
          ['OS=="linux" and target_arch=="arm" and chromeos==0', {
            ...
            'sysroot%': '<!(cd <(webrtc_depth) && pwd -P)/build/linux/debian_wheezy_arm-sysroot',
          }], # OS=="linux" and target_arch=="arm" and chromeos==0

          ['OS=="linux" and ((branding=="Chrome" and buildtype=="Official" and chromeos==0) or use_sysroot==1)' , {
            'conditions': [
              ['target_arch=="x64"', {
                'sysroot%': '<!(cd <(webrtc_depth) && pwd -P)/build/linux/debian_wheezy_amd64-sysroot',
              }],
              ['target_arch=="ia32"', {
                'sysroot%': '<!(cd <(webrtc_depth) && pwd -P)/build/linux/debian_wheezy_i386-sysroot',
              }],
          ],
          }], # OS=="linux" and branding=="Chrome" and buildtype=="Official" and chromeos==0

          ['OS=="linux" and target_arch=="mipsel"', {
            'sysroot%': '<!(cd <(webrtc_depth) && pwd -P)/build/linux/debian_wheezy_mips-sysroot',
          }],
        ],
      },
      ...
      'wix_path%': '<(webrtc_depth)/third_party/wix',
      ...
      'conditions': [
        ...
        ['target_arch=="x64"', {
          'sas_dll_path%': '<(webrtc_depth)/third_party/platformsdk_win7/files/redist/amd64',
        }, {
          'sas_dll_path%': '<(webrtc_depth)/third_party/platformsdk_win7/files/redist/x86',
        }],

        ['sysroot!=""', {
          'pkg-config': '<(chroot_cmd) <(webrtc_depth)/build/linux/pkg-config-wrapper "<(sysroot)" "<(target_arch)" "<(system_libdir)"',
        }, {
    ...
    'directx_sdk_default_path': '<(webrtc_depth)/third_party/directxsdk/files',
    ...
    'libjpeg_ijg_gyp_path': '<(webrtc_depth)/third_party/libjpeg/libjpeg.gyp',
    'libjpeg_turbo_gyp_path': '<(webrtc_depth)/third_party/libjpeg_turbo/libjpeg.gyp',
    ...
    'conditions': [
      ...
      ['OS=="android"', {
        ...
        'variables': {
          'variables': {
            ...
            # from GYP.
            'android_sdk_root%': '<!(cd <(webrtc_depth) && pwd -P)/third_party/android_tools/sdk/',
            ...
            # in the GYP files.
            'android_ndk_absolute_root%': '<!(cd <(webrtc_depth) && pwd -P)/third_party/android_tools/ndk/',
      ...
      ['OS=="mac" or OS=="ios"', {
        ...
        'conditions': [
          ...
          ['branding=="Chrome" and buildtype=="Official" and OS=="mac"', {
            'mac_sdk%': '<!(python <(webrtc_depth)/build/mac/find_sdk.py --verify <(mac_sdk_min) --sdk_path=<(mac_sdk_path))',
          }, {
            'mac_sdk%': '<!(python <(webrtc_depth)/build/mac/find_sdk.py <(mac_sdk_min))',
          }],
          ['branding=="Chrome" and buildtype=="Official"', {
      ...
      ['OS == "ios"', {
        ...
        'grit_whitelist%': '<(webrtc_depth)/build/ios/grit_whitelist.txt',
          ...
          'conditions': [
            ['OS!="win"', {
              'variables': {
                'conditions': [
                  ['OS=="mac" or OS=="ios"', {
                    'clang_lib_path%': '<!(cd <(webrtc_depth) && pwd -P)/third_party/llvm-build/Release+Asserts/lib/libFindBadConstructs.dylib',
                  }, { # OS != "mac" or OS != "ios"
                    'clang_lib_path%': '<!(cd <(webrtc_depth) && pwd -P)/third_party/llvm-build/Release+Asserts/lib/libFindBadConstructs.so',
                  }],
                ],
              },
    ...
    'angle_path': '<(webrtc_depth)/third_party/angle',
  ...
  'target_defaults': {
    'variables': {
      ...
      'win_exe_compatibility_manifest%':
          '<(webrtc_depth)\\build\\win\\compatibility.manifest',
    ...
    'conditions': [
      ...
      ['(OS=="mac" or OS=="ios") and asan==1', {
        'dependencies': [
          '<(webrtc_depth)/build/mac/asan.gyp:asan_dynamic_runtime',
        ],
      }],
      ['OS=="win" and asan==1 and component=="shared_library"', {
        'dependencies': [
          '<(webrtc_depth)/build/win/asan.gyp:asan_dynamic_runtime',
        ],
      }],
      ['OS=="linux" and use_allocator!="none" and clang_type_profiler==1', {
        ...
        'dependencies': [
          '<(webrtc_depth)/base/allocator/allocator.gyp:type_profiler',
        ],
      }],
      ['branding=="Chrome"', {
      ...
      ['clang==1 or host_clang==1', {
        ...
        # define.
        'defines': ['CR_CLANG_REVISION=<!(python <(webrtc_depth)/tools/clang/scripts/update.py --print-revision)'],
      }],
      ['enable_rlz==1', {
        'defines': ['ENABLE_RLZ'],
      }],
      ...
      ['kasko==1', {
        ...
        'include_dirs': [
          '<(webrtc_depth)/third_party/kasko/include',
        ],
      }],
      ['OS=="win"', {
        'defines': [
        ...
        'include_dirs': [
          '<(webrtc_depth)/third_party/wtl/include',
        ],
        'conditions': [
          ['win_z7!=0', {
            'msvs_settings': {
    ...
    'configurations': {
      ...
      'Common_Base': {
        ...
        'msvs_configuration_attributes': {
          'OutputDirectory': '<(webrtc_depth)\\build\\<(build_dir_prefix)$(ConfigurationName)',
      ...
      'target_defaults': {
        ...
        'conditions': [
          ['use_sanitizer_options==1 and ((OS=="linux" and (chromeos==0 or target_arch!="ia32")) or OS=="mac")', {
            'dependencies': [
              '<(webrtc_depth)/build/sanitizers/sanitizers.gyp:sanitizer_options',
            ],
          }],
        ],
      },
      ...
      'target_defaults': {
        ...
        'conditions': [
          ...
          ['sysroot!=""', {
            'target_conditions': [
              ['_toolset=="target"', {
                ...
                'ldflags': [
                  '--sysroot=<(sysroot)',
                  '<!(<(webrtc_depth)/build/linux/sysroot_ld_path.sh <(sysroot))',
                ],
              }]]
          }],
          ['clang==1', {
          ...
          ['use_instrumented_libraries==1', {
            'dependencies': [
              '<(webrtc_depth)/third_party/instrumented_libraries/instrumented_libraries.gyp:instrumented_libraries',
            ],
          }],
          ['use_prebuilt_instrumented_libraries==1', {
            'dependencies': [
              '<(webrtc_depth)/third_party/instrumented_libraries/instrumented_libraries.gyp:prebuilt_instrumented_libraries',
            ],
          }],
          ['use_custom_libcxx==1', {
            'dependencies': [
              '<(webrtc_depth)/buildtools/third_party/libc++/libc++.gyp:libcxx_proxy',
            ],
          }],
          ...
          ['linux_use_bundled_binutils==1', {
            'cflags': [
              '-B<!(cd <(webrtc_depth) && pwd -P)/<(binutils_dir)',
            ],
          }],
              ...
              ['component=="static_library"', {
                'target_conditions': [
                  ['use_native_jni_exports==0', {
                    ...
                    'ldflags': [
                      '-Wl,--version-script=<!(cd <(webrtc_depth) && pwd -P)/build/android/android_no_jni_exports.lst',
                    ],
                  }],
                ],
              }],
      ...
      'target_defaults': {
        ...
        'msvs_settings': {
          ...
          'conditions': [
            ...
            ['clang==1 and "<!(python <(webrtc_depth)/build/win/use_ansi_codes.py)"=="True"', {
              'VCCLCompilerTool': {
                'AdditionalOptions': [
                ...
                ['asan==1', {
                  ...
                  'VCCLCompilerTool': {
                    ...
                    'AdditionalIncludeDirectories': [
                      ...
                      # back.
                      '<(webrtc_depth)/<(make_clang_dir)/lib/clang/<!(python <(webrtc_depth)/tools/clang/scripts/update.py --print-clang-version)/include_sanitizer',
                    ],
                  },
                  'VCLinkerTool': {
                    'AdditionalLibraryDirectories': [
                      # TODO(hans): If make_clang_dir is absolute, this breaks.
                      '<(webrtc_depth)/<(make_clang_dir)/lib/clang/<!(python <(webrtc_depth)/tools/clang/scripts/update.py --print-clang-version)/lib/windows',
                    ],
                  },
                  'target_conditions': [
                    ['component=="shared_library"', {
  ...
  'xcode_settings': {
    ...
    'SYMROOT': '<(webrtc_depth)/xcodebuild',
  },
}
