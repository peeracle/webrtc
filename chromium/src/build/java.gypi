...
{
  'dependencies': [
    '<(webrtc_depth)/build/android/setup.gyp:build_output_dirs'
  ],
  'variables': {
    ...
    ['has_java_resources == 1', {
      ...
      'actions': [
        # Generate R.java and crunch image resources.
        {
          ...
          'variables': {
            'android_manifest': '<(webrtc_depth)/build/android/AndroidManifest.xml',
          ...
          'inputs': [
            ...
            '<(webrtc_depth)/build/android/gyp/generate_v14_compatible_resources.py',
      ...
      'actions': [
        {
          ...
          'inputs': [
            '<(webrtc_depth)/third_party/proguard/lib/proguard.jar',
            '<(webrtc_depth)/build/android/gyp/util/build_utils.py',
            '<(webrtc_depth)/build/android/gyp/proguard.py',
            '<(javac_jar_path)',
            '<(proguard_config)',
          ],
          'outputs': [
          ...
          'action': [
            'python', '<(webrtc_depth)/build/android/gyp/proguard.py',
            '--proguard-path=<(webrtc_depth)/third_party/proguard/lib/proguard.jar',
  ...
  'actions': [
    ...
    },
    {
      ...
      'inputs': [
        '<(webrtc_depth)/build/android/gyp/util/build_utils.py',
        '<(webrtc_depth)/build/android/gyp/util/md5_check.py',
        '<(webrtc_depth)/build/android/gyp/jar_toc.py',
        '<(jar_final_path)',
      ],
      'outputs': [
        '<(jar_final_path).TOC',
      ...
      'action': [
        'python', '<(webrtc_depth)/build/android/gyp/jar_toc.py',
